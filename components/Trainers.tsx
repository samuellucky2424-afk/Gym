    ...
    <section id="trainers" className="py-24 bg-zinc-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div>
            <h2 className="text-red-600 font-black text-sm uppercase tracking-[0.3em] mb-4">Elite Coaching</h2>
            <h3 className="text-4xl sm:text-5xl font-black text-white tracking-tighter">MEET THE EXPERTS</h3>
          </div>
          ...
          {TRAINERS.map((trainer) => (
            <div key={trainer.id} className="group relative">
              <div className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-zinc-800">
                ...
                <div className="absolute bottom-6 left-6 right-6 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                  <div className="flex gap-4">
                    <a href="#" className="p-2 bg-white/10 backdrop-blur-md rounded-full text-white hover:bg-red-600 transition-colors">
                      <Instagram size={18} />
                    </a>
                    <a href="#" className="p-2 bg-white/10 backdrop-blur-md rounded-full text-white hover:bg-red-600 transition-colors">
                      <Twitter size={18} />
              ...
              <div className="mt-6 text-center">
                <h4 className="text-2xl font-black text-white mb-1 group-hover:text-red-600 transition-colors">{trainer.name}</h4>
                <p className="text-red-600 font-bold text-sm uppercase tracking-widest">{trainer.role}</p>
                <p className="text-zinc-500 text-sm mt-2 font-medium">{trainer.specialty}</p>
              </div>
            </div>
          ))}
        </div>
